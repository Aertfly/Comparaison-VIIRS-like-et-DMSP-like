...
class Clusters():
    ...
    def optimal_k(self, X, dataset_name, path, max_clusters=15):  
        inertias = []
        K = range(1, max_clusters + 1)
        
        for k in K:
            km = Clusters(n_clusters=k, ntimes=1, dist=self.dist_name, norm=self.norm_name,
                        dataset_name=dataset_name, path=path, x_range=self.x_range,
                        aois=self.aois, mask=self.mask, city_name=self.city_name, ntl_type=self.ntl_type)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--name","-n", help="Nom du jeu de données à utiliser")
    parser.add_argument("--ntl_type","-t", help="Type de jeu de données à utiliser")
    parser.add_argument("-c","--nbCluster",nargs='+',default=[],help="Liste des clusters")
    parser.add_argument("--noResize","-nr",action='store_true',help ="Indique au programme de ne pas utiliser les données redimensionnées")
    parser.add_argument("--noShow","-ns",action='store_false',help ="Indique à la fin que le graphique ne doit pas être affiché")
    ...
    if not args.nbCluster:
        n_clusters = 1  # Valeur par défaut
    else:
        n_clusters = int(args.nbCluster[0])  # Utilisez la première valeur de la liste
    ...
    km = Clusters(**params)
    optimal_clusters = km.optimal_k(ntls, dataset_name=args.name + '/' + args.ntl_type, path=km.src_path)
    clusters = [optimal_clusters]

    print("Nombre de clusters étudié", clusters)
    ...
    for norm in ['local']:
        for i in clusters:
            ...
            km = Kmeans(**params)
            km(ntls, samples_for_distance_matrix=1000, shape=data.getShape(), show=show, n_clusters=i)