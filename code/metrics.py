...
def r2_global(y_true, y_pred):
    ...
    # Average R2 per year on each folds
    r2_mean = r2_.mean(axis=1).mean()  # .mean()
    # Variations on folds
    r2_std_on_folds = r2_.mean(axis=1).std()  # .std()
    # Average variations per year
    r2_std_on_ts = r2_.std(axis=1).mean()

    return r2_mean, r2_std_on_folds, r2_std_on_ts
...
def global_scores(observations_d, prediction_d):
    ...
    for metric_name, metric in zip(['MAE', 'DIS', 'R2'], [mae_global, dis_global, r2_global]):
        ...
        for (key, pred), (_, true) in zip(prediction_d.items(), observations_d.items()):
            ...
            error, std_on_folds, std_on_ts = metric(true, pred)
            display = f'\t\t{key} : {error:.6} +- {std_on_folds:.6} '
            if std_on_ts != 0:
                display += f'TS +- {std_on_ts:.6}'
            print(display)
